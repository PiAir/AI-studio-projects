document.addEventListener('DOMContentLoaded', function () {
    ...
    function generateReport() {
        const currentReportOutput = document.getElementById('report-output'); // Re-fetch or ensure it's valid
        if (!currentReportOutput) {
            console.error("generateReport Error: DOM element with ID 'report-output' not found.");
            return "";
        }
        ...
        currentReportOutput.textContent = reportMd.trim() ? reportMd : "Er zijn nog geen selecties gemaakt.";
        return reportMd;
    }
    ...
    nextButton.addEventListener('click', () => {
        ...
        if (currentPageIndex < pageOrder.length - 1) {
            if (pageOrder[currentPageIndex] === 'page-end') {
                ...
                // but as a fallback, go to report
    ...
    viewReportButton.addEventListener('click', () => {
        collectPageData(pageOrder[currentPageIndex]); // Collect data from current page (page-end)
        currentPageIndex = pageOrder.indexOf('page-report');
        generateReport();
        showPage(currentPageIndex);
    });
    ...
    if (downloadReportButton) {
        downloadReportButton.addEventListener('click', () => {
            const reportTextForDownload = generateReport();
